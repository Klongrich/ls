...
void	handel_printing(char **all_files_or_directories, t_flags *flags) {
	...
	if(flags->color & !flags->l & isatty(STDOUT_FILENO))
		print_color(all_files_or_directories, 0, 0);    
	if (flags->l || flags->t || flags->color) 
		cut_file_path_from_files_or_dirs(all_files_or_directories);
	if(!flags->l & !flags->color || (flags->color & !isatty(STDOUT_FILENO) & !flags->l))
...
char 	**read_dir(char *dir_path, t_flags *flags){
	...
	repo_or_file_count = get_repo_or_file_count(dir_path, flags->a, flags->l, flags->f);	
