...
int	get_spec(char *str, int count) {
	int res;
...
int	compare_strs_lowercase(char *str1, char *str2){
	...
	while (str1[count]) {
		lower_case_all_characters_one[count] = ft_tolower(str1[count] - get_spec(str1, count));
		count++;
	}
	count = 0;
	while(str2[count]){
		lower_case_all_characters_two[count] = ft_tolower(str2[count] - get_spec(str2, count));
		count++;
	}
