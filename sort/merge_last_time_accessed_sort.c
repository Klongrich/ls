...
int	check_letter_l(t_merge_data data, int i, int j, char *arr[]) {
...
int	check_time_data_l(t_merge_data data, int i, int j, char *arr[]) {
	if (data.right.st_atime == data.left.st_atime) {
		if(data.right.st_atimespec.tv_nsec == data.left.st_atimespec.tv_nsec) {
			return check_letter_l(data, i, j, arr);
	 	} else {
			if (data.right.st_atimespec.tv_nsec < data.left.st_atimespec.tv_nsec) {
				arr[data.k] = data.L[i];
				return (1);
...
void	put_rest_n1_l(t_merge_data data, t_merge *merge, int i, char *arr[]) {
	while (i < merge->n1) 
		arr[data.k++] = data.L[i++];
}

void    put_rest_n2_l(t_merge_data data, t_merge *merge, int j, char *arr[]) {
	while (j < merge->n2)
		arr[data.k++] = data.R[j++];
}

void	merge_sides_l(char *arr[], char **L, char **R, t_merge *merge) {
	int i;
	int j;
	int res;
	t_merge_data data;
	...
	put_rest_n2_l(data, merge, j, arr);
	free(L);
	free(R);
}

void merge_data_l(char* arr[], t_merge *merge) {
	int i;
	int j;
	char **L;
	char **R;
	...
	merge_sides_l(arr, L, R, merge);
}

void	init_t_merge_tt(t_merge *merge) {
	merge->m = 0;
	merge->l = 0;
	merge->r = 0;
	merge->n1 = 0;
